...
int toymcpy8 ( int argc, char *argv[] )
{
  Int_t kCollider = -1; // for LHC
  Double_t gmultCollider = 0; // default zero
  Int_t gNbinCollider = 0;

  GenerUtil::init_defaults();

  cout << "mult LHC:" << GenerUtil::gmultLHC << endl;

  if (SysUtil::isSet("-rhic", argc, argv))
  {
    kCollider     = 0;
    gmultCollider = GenerUtil::gmultRHIC;
    gNbinCollider = GenerUtil::gNbinRHIC;
  }

  if (SysUtil::isSet("-lhc", argc, argv))
  {
    kCollider     = 1;
    gmultCollider = GenerUtil::gmultLHC;
    gNbinCollider = GenerUtil::gNbinLHC;
  }
  ...
  {
    cerr << "[e] Collider settings not set."
         << " Mult collider: " << gmultCollider
         << endl;
...
// Extract settings to be used in the main program.
  ...
  for (int iEvent = 0; iEvent < nEvent; ++iEvent) {
    ...
    //void GenerUtil::bg_particle(Int_t mult, Int_t collider, TLorentzVector &v)
