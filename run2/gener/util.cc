bool isSet(char *what, int argc, char **argv)
{
...
char *getArg(char *what, int argc, char **argv)
{
  for (int i = 0; i < argc; i++)
    {
      if (strcmp(what, argv[i]) == 0)
...
static TF1 *gfunbg = 0;
static Double_t gmean_pt_LHC = 0.7; // LHC energy (2.76 TeV)
static Double_t gmean_pt_RHIC = 0.5; //RHIC energy (0.2 TeV)
static Double_t gmultLHC = 6100;  //  LHC 0-5% in |eta|<1 is ~8000
static Double_t gmultRHIC = 2800; // RHIC 0-5% in |eta|<1

static Int_t gNbinLHC  = 1500; // this is for 0-10%! - make it consistent
static Int_t gNbinRHIC =  500;

//static TRandom3 *grnd = new TRandom3;
static TRandom *grnd = new TRandom;

static Int_t gpyParticleOffset = 10001;
...
void add_particles(std::vector <fj::PseudoJet> &to, std::vector <fj::PseudoJet> &from, double rotate_phi = 0)
{
...
bool add_pythia_particles(py::Pythia &pythia, std::vector <fj::PseudoJet> &parts, double etaMax = 4., UInt_t offset = 0)
{
