...
def tbit(ttype):
	d = dict([('kOther' , 1),('kCINT7' , 2),('kEJ1' , 4),('kEG1' , 8),('kDJ1' , 16), ('kDG1' , 33)])
	return d[ttype]

def get_jets(indir, var='', cuts='', bwidth=5, xlow= 0, xhigh=200):
...
def get_n_events(indir, ttype, bwidth=100, xlow= 0, xhigh=200):
...
def main(dname):
	...
	for fdet in [0, 1]:
		for jets in [ 'jets00.fE', 'jets02.fE', 'jets04.fE' ]:
			for ttype in ['kCINT7', 'kEJ1', 'kEG1', 'kDJ1', 'kDG1']:
				...
				h = get_jets(dname, jets, '(trig.type & {})&&({})'.format(bit, detcondition))
...
def draw():
	...
	ls.draw_all(logy=True)
	lsr.draw_all(logy=True, maxy=2.e3)
