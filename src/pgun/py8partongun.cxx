...
void PGun::SetSpectrum(TGraphErrors *gr, int id)
{
	Int_t n = gr->GetN();
	Double_t *x = gr->GetX();
	...
	for (Int_t i = 0; i < n; i++)
	{
		...
		else
		{
			if (i + 1 < n)
			{
				abins[i] = x[i] - (x[i+1] - x[i])/2.;
			}
			else
			{
				abins[i] = x[i] - (x[i] - x[i-1])/2.;
			}
		}
	}
	abins[n] = x[n-1] + ex[n-1]/2.;
	//TH1F (const char *name, const char *title, Int_t nbinsx, const Double_t *xbins)
	TH1D *h = new TH1D(gr->GetName(), gr->GetTitle(), n, abins.GetArray());
	for (Int_t i = 0; i < n; i++)
	{
		Int_t ibin = i + 1;
		h->SetBinContent(ibin, y[i]);
		h->SetBinError(ibin, ey[i]);
	}
	SetSpectrum(h, id);
}
