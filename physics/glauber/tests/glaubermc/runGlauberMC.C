void runGlauberMC(Bool_t doPartProd=0,Int_t option=0,Int_t N=10000)
{
  //load libraries
  gSystem->Load("libVMC");
  gSystem->Load("libPhysics");
  gSystem->Load("libTree");
  gSystem->Load("libAliGlauber");
  gSystem->Load("libGlauberMC");
  ...
  //  GlauberPy::runAndSaveNucleons(10000,"Pb","Pb",72);
  Double_t mind=0.4;
  //  GlauberPy::RunAndSaveNtuple(nevents,sysA,sysB,signn,mind);
  Double_t r=6.62;
  Double_t a=0.546;
  const char *fname="glau_pbpb_ntuple.root";

  GlauberPy mcg(sysA,sysB,signn);
